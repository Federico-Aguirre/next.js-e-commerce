...
export default function SuccessScreen() {
  ...
  const searchParams = useLocalSearchParams<{ payment_id?: string; status?: string }>();
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="bg-gray-50 p-4 justify-center items-center">
      <View className="bg-white max-w-md w-full p-8 rounded-2xl shadow-md border border-gray-100 items-center gap-y-6">
        {errorVerificacion ? (
          <>
            <Text className="text-6xl">❌</Text>
            <Text className="text-2xl font-black text-gray-900 text-center">Error de Verificación</Text>
          </>
        ) : (
          <>
            <Text className="text-6xl">✅</Text>
            <Text className="text-2xl font-black text-gray-900 text-center">¡Muchas gracias por tu compra!</Text>
            {guardando ? (
              <ActivityIndicator color="#4f46e5" />
            ) : (
              <Text className="text-gray-500 text-sm">Comprobante N°: {paymentId || 'N/A'}</Text>
            )}
          </>
        )}

        <Pressable onPress={() => router.push('/historial')} className="w-full bg-indigo-600 h-12 rounded-lg justify-center items-center">
