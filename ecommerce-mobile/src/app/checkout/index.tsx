...
import { View, Text, ScrollView, Image, Pressable, ActivityIndicator, Linking, Alert, TextInput } from 'react-native';
...
export default function CheckoutScreen() {
  ...
  const obtenerTokenTarjeta = async () => {
    console.log('📌 Ejecutando tokenización con credenciales activas. Public Key:', process.env.EXPO_PUBLIC_MERCADOPAGO_PUBLIC_KEY);
    const publicKey = process.env.EXPO_PUBLIC_MERCADOPAGO_PUBLIC_KEY;

    if (!publicKey) {
      throw new Error('Falta configurar EXPO_PUBLIC_MERCADOPAGO_PUBLIC_KEY en tus variables de entorno.');
    }
    ...
    if (!response.ok || !data.id) {
      ...
      throw new Error(data?.cause?.[0]?.description || 'Datos de tarjeta inválidos o incompletos.');
    }
  ...
  const handleProcessPayment = async (paymentMethod: 'mercadopago' | 'stripe') => {
    ...
    const targetUserId = currentUser?.id || (currentUser as any)?._id || (currentUser as any)?.sub;
    ...
    try {
      ...
      if (paymentMethod === 'mercadopago') {
        console.log('🔄 Generando Token de tarjeta en Mercado Pago...');
        const cardToken = await obtenerTokenTarjeta();

        console.log('🚀 Procesando pago en el servidor (/api/payment/process)...');
        ...
        if (!response.ok || !data.success) {
          throw new Error(data.error || 'El pago no pudo ser procesado por Mercado Pago.');
        }

        console.log('🎉 ¡Pago aprobado con éxito!', data);
      ...
      if (paymentMethod === 'stripe') {
        const savedOrderId = await AsyncStorage.getItem('last_pending_order_id');
        if (savedOrderId) {
    ...
    return (
      <View className="flex-1 bg-gray-50 items-center justify-center p-6">
        <View className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm w-full max-w-sm items-center">
          ...
          <Text className="text-xl font-bold text-gray-800 mb-2 text-center">Tu carrito está vacío</Text>
          ...
          >
            <Text className="text-sm font-bold text-white">Volver al catálogo</Text>
          </Pressable>
        </View>
      </View>
    );
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50 px-4 py-6">
      <View className="max-w-4xl mx-auto pb-12 gap-y-6">
        <Text className="text-2xl font-black text-gray-900">Tu Carrito de Compras</Text>
        ...
        <View className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 gap-y-4">
          {cart.map((item) => {
            ...
            const imageUrl = item.image && item.image.trim() !== '' ? item.image : 'https://placehold.co/150x150?text=No+Image';

            return (
              <View key={`${itemRealId}-${item.size}`} className="flex-row items-center border-b border-gray-100 pb-4 last:border-b-0 last:pb-0">
                ...
                <View className="flex-1 justify-between gap-y-1">
                  ...
                  <Text className="text-xs text-gray-400 font-semibold">
                    Talle: <Text className="text-gray-600 font-normal uppercase">{item.size}</Text>
                  </Text>
                  <View className="flex-row items-center justify-between mt-2">
                    <View className="flex-row items-center border border-gray-200 rounded-md bg-gray-50">
                      <Pressable onPress={() => handleDecreaseQuantity(item)} className="px-3 py-1 active:bg-gray-200 rounded-l-md">
                        <Text className="text-gray-600 font-bold text-base">−</Text>
                      </Pressable>
                      <Text className="px-2 text-sm font-bold text-gray-800 min-w-6 text-center">{item.quantity}</Text>
                      <Pressable onPress={() => addToCart(item)} className="px-3 py-1 active:bg-gray-200 rounded-r-md">
                        <Text className="text-gray-600 font-bold text-base">+</Text>
                      </Pressable>
                    </View>
                    <Pressable onPress={() => removeFromCart(itemRealId, item.size)} className="p-1">
                      <Text className="text-xs font-semibold text-red-500">Eliminar</Text>
                    </Pressable>
                  </View>
                </View>
                <Text className="text-sm font-black text-gray-900 ml-2">${(item.price * item.quantity).toFixed(2)}</Text>
              </View>
            );
          })}
        </View>
        ...
        <View className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm gap-y-4">
          <Text className="text-lg font-bold text-gray-900 border-b border-gray-100 pb-3">Resumen del pedido</Text>
          <View className="gap-y-2 text-sm">
            <View className="flex-row justify-between items-center">
              <Text className="text-gray-600 text-sm">Subtotal</Text>
              <Text className="font-semibold text-gray-900 text-sm">${subtotal.toFixed(2)}</Text>
            </View>
            <View className="flex-row justify-between items-center">
              <Text className="text-gray-600 text-sm">Costo de envío estimado</Text>
              <Text className="font-semibold text-gray-900 text-sm">${shipping.toFixed(2)}</Text>
            </View>
            <View className="flex-row justify-between items-center border-t border-gray-100 pt-3 mt-1">
              ...
              <Text className="font-black text-gray-900 text-xl">${total.toFixed(2)}</Text>
            </View>
          </View>
          ...
          {showMpForm ? (
            <View className="border-t border-gray-100 pt-4 gap-y-3">
              <View className="flex-row justify-between items-center mb-1">
                <Text className="text-xs font-bold text-sky-600 uppercase">Tarjeta de Crédito / Débito</Text>
                <Pressable onPress={handleFillTestData} className="bg-sky-50 px-2 py-1 rounded">
                  <Text className="text-xs text-sky-600 font-bold">⚡ Usar Tarjeta APRO</Text>
                </Pressable>
              </View>

              <View>
                <Text className="text-xs font-bold text-gray-600 mb-1">Número de Tarjeta</Text>
                <TextInput
              ...
              <View>
                <Text className="text-xs font-bold text-gray-600 mb-1">Nombre del Titular</Text>
                <TextInput
                  placeholder="APRO"
              ...
              <View className="flex-row gap-x-2">
                <View className="flex-1">
                  <Text className="text-xs font-bold text-gray-600 mb-1">Mes (MM)</Text>
                  <TextInput
                    placeholder="11"
                    maxLength={2}
                    keyboardType="numeric"
                    value={cardData.expirationMonth}
                    onChangeText={(text) => setCardData({ ...cardData, expirationMonth: text })}
                    className="w-full text-sm p-3 border border-gray-200 rounded-lg text-gray-800 bg-gray-50 text-center"
                  />
                </View>
                <View className="flex-1">
                  <Text className="text-xs font-bold text-gray-600 mb-1">Año (AA)</Text>
                  <TextInput
                    placeholder="28"
                    maxLength={2}
                    keyboardType="numeric"
                    value={cardData.expirationYear}
                    onChangeText={(text) => setCardData({ ...cardData, expirationYear: text })}
                    className="w-full text-sm p-3 border border-gray-200 rounded-lg text-gray-800 bg-gray-50 text-center"
                  />
                </View>
                <View className="flex-1">
                  <Text className="text-xs font-bold text-gray-600 mb-1">CVV</Text>
                  <TextInput
                    placeholder="123"
                    maxLength={4}
                    keyboardType="numeric"
              ...
              <Pressable onPress={() => setShowMpForm(false)} className="py-2 items-center">
                <Text className="text-xs text-gray-400 font-semibold">Cancelar / Volver</Text>
              </Pressable>
            </View>
          ) : (
            <View className="gap-y-3 pt-2">
              ...
              >
                <Text className="text-white text-sm font-bold">Pagar con Mercado Pago</Text>
              </Pressable>
              ...
              >
                {isProcessing ? <ActivityIndicator color="#ffffff" /> : <Text className="text-white text-sm font-bold">Pagar con Stripe</Text>}
              </Pressable>
            </View>
          )}
        </View>
